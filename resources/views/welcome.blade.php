                    ...
                    <form id="searchFilterForm" action="#" method="POST">
                        ...
                        <div class="row">
                            @foreach($fields as $field)
                                <div class="col-12">
                                    <input type="checkbox" name="selected_fields[]" value="{{ $field }}" id="{{ $field }}" {{ isset($configurations) && in_array( $field,$configurations['configurations']??[]) ? 'checked':''}}>
                                    <label for="{{ $field }}">
