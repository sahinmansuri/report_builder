...
<table class="table table-bordered" id="{{$field->field_name}}Table">
    ...
    <tbody>
        @forelse ($configurations['filter'][$field->field_name??null]??[] as $index => $main_field)
            <tr>
                @foreach ($children as $key => $child)
                    <td>
                        {{-- @dd($configurations['filter'],$field->field_name,$index,$child->field_name,$child); --}}
                        @includeFirst(["components.{$child->field_type}", "components.text"], [
                            'field' => $child,
                            'name' => "$field->field_name[$index][{$child->field_name}]",
                            "main_field_name" => $field->field_name,
                            "value"=>$configurations['filter'][$field->field_name??null][$index??null][$child->field_name]??null])
                    </td>
                @endforeach
                @if(!$loop->first)
                    <td>
            ...
            <tr>
                @foreach ($children as $key => $child)
                    <td>
                        @includeFirst(["components.{$child->field_type}", "components.text"], [
                            'field' => $child,
                             'name' => "$field->field_name[0][{$child->field_name}]",
                             "value"=>null])
                    </td>
                @endforeach
                <td>
